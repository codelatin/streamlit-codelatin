...
def get_data_from_excel():
    df = pd.read_excel(
        io="pages/doc_ventas/supermarkt_sales.xlsx",
        engine="openpyxl",
        sheet_name="Sales",
        skiprows=3,
        usecols="B:R",
...
st.title(":bar_chart: Dashboard 2024")
st.markdown("##")
...
with right_column:
    st.subheader("promedio ventas por transacción 2024")
    st.subheader(f" USD $ {average_sale_by_transaction}")

st.markdown("------")

# Agregar un gráfico de dispersión para visualizar la relación entre dos variables
scatter_plot = px.scatter(df_selection, x="Rating", y="Total", color="Gender", title="Relación entre Calificación y Ventas", template="plotly_white")
...
fig_product_sales = px.bar(
    ...
    color_discrete_sequence=["#ccc"] * len(sales_by_product_line),
...
# Agregar un gráfico de pastel para mostrar la distribución de ventas por ciudad
...
# Mostrar un resumen de las ventas por tipo de cliente y género
sales_summary = df_selection.groupby(["Customer_type", "Gender"]).agg({"Total": ["sum", "mean"]})
st.subheader("Resumen de Ventas por Tipo de Cliente y Género")
st.write(sales_summary)

# Ocultar estilos predeterminados de Streamlit
hide_st_style = """
          <style>
